...
impl Rectangle {
    fn new(width: u32, height: u32, pos_left: u32, pos_top: u32, id: u32) -> Rectangle {
        let start = Coordinate { x: pos_left, y: pos_top };
        let end = Coordinate { x: pos_left + width, y: pos_top + height };
        Rectangle {
            width,
            height,
            pos_left,
...
fn print_rectangles(rectangles: &Vec<Rectangle>) {
    for i in 0..rectangles.len() {
        if let Some(r) = rectangles.get(i) {
            println!("#{} @ {},{}: {}x{}",r.id,r.pos_left,r.pos_top,r.width,r.height);
        }
    }
}
...
fn main() {
    ...
    for (nr, line) in buf_reader.lines().enumerate() {
        ...
        if let Some(s) = v.get(1) {
            ...
            rectangles.push(Rectangle::new(width,height,pos_left,pos_top,(nr+1) as u32));
        }
    }
...
//     print_rectangles(&rectangles);
    ...
    println!("The overall overlapping area in square inches: {}", overlapping_area);
    println!("The ID of the one rectangle without overlapping area: {}", id_not_overlapping);
}
